# модуль стандартной библиотеки Python, предназначенный для работы с аргументами командной строки
import argparse
# модуль для логирования информации (https://khashtamov.com/ru/python-logging/)
import logging
import random
# модуль для работы с сокетами (https://habr.com/ru/post/149077/)
import socket
# Модуль sys обеспечивает доступ к некоторым переменным и функциям, взаимодействующим с интерпретатором python. (https://pythonworld.ru/moduli/modul-sys.html)
import sys
import time
...
# if пользователь ввел один аргумент то выводим справку и выходим из программы
if len(sys.argv) <= 1:
    parser.print_help()
    sys.exit(1)

# обязательно нужно ввести host
if not args.host:
    print("Host required!")
    parser.print_help()
    sys.exit(1)
...
def init_socket(ip):
    # указываеем семейство протоколов создаваемого сокета
